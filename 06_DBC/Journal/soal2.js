class SayaTubeVideo {
  ...
  increasePlayCount(count) {
    if (typeof count !== "number")
      throw new Error("Play count harus berupa angka.");
    if (count <= 0) throw new Error("Play count tidak boleh negatif atau nol.");
    if (count > 25000000)
      throw new Error(
        "Penambahan play count maksimal 25.000.000 per panggilan."
      );
...
class SayaTubeUser {
  ...
  addVideo(video) {
    ...
    if (!(video instanceof SayaTubeVideo))
      throw new Error("Objek harus bertipe SayaTubeVideo.");
...
try {
  ...
  try {
    const testVideo = new SayaTubeVideo("Review Film Terbaik Sepanjang Masa");
    testVideo.increasePlayCount(Number.MAX_SAFE_INTEGER);
  } catch (error) {
    console.error(
      `Terjadi kesalahan saat menaikkan play count: ${error.message}`
    );
  }
} catch (error) {
