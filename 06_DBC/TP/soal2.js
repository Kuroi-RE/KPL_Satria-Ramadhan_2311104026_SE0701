class SayaTubeVideo {
  constructor(title) {
    if (typeof title !== "string" || title.length === 0 || title.length > 100) {
      throw new Error(
        "Title must be a non-empty string with a maximum of 100 characters"
      );
    }
  ...
  IncreasePlayCount(count) {
    try {
      if (
        typeof count !== "number" ||
        count <= 0 ||
        count > this.MAX_PLAY_COUNT
      ) {
        throw new Error(
          `Play count must be a number between 1 and ${this.MAX_PLAY_COUNT}`
        );
      }

      if (this.playCount + count > this.MAX_INTEGER) {
        throw new Error(
          "Integer overflow detected! Cannot increase play count."
        );
      }
...
const video = new SayaTubeVideo("Satria Ramadhan");
